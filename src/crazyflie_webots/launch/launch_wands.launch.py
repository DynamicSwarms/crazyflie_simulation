...
def create_wand_controllers(context):
    ...
    webots_use_tcp = LaunchConfiguration("webots_use_tcp").perform(context).lower() in [
        "true",
        "1",
        "yes",
        "on",
...
def generate_launch_description():
    ...
    webots_port_launch_arg = DeclareLaunchArgument(
        name="webots_port", default_value="1234", description="Webots port."
    )
    ...
    return LaunchDescription(
        [
            robots_yaml_launch_arg,
            webots_port_launch_arg,
            webots_use_tcp_launch_arg,
            webots_tcp_ip_launch_arg,
            OpaqueFunction(function=create_wand_controllers),
        ]
    )
