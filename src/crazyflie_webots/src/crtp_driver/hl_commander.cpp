    ...
    crazyflie_interfaces::srv::Land::Response::SharedPtr response)
{
    ...
    if (auto webots_driver = m_webots_driver.lock()) {
        ...
        webots_driver->set_target(current_pose.translation(), request->yaw);
    }
}
    ...
    crazyflie_interfaces::srv::GoTo::Response::SharedPtr response)
{
    ...
    if (auto webots_driver = m_webots_driver.lock()) {
        if (request->relative) {
            ...
            webots_driver->set_target(target, current_yaw + request->yaw);
        } else {
            Eigen::Vector3d target(request->goal.x, request->goal.y, request->goal.z);
            webots_driver->set_target(target, request->yaw);
        }
    }
}